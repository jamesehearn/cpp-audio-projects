/*
	...
	Date: 12/22/2013
	Description: Digital Delay Line
...
bool __stdcall CDDLModule::prepareForPlay()
{
	...
	m_nBufferSize = 2*m_nSampleRate;	//Two secodns of delay @f_s
...
bool __stdcall CDDLModule::processAudioFrame(float* pInputBuffer, float* pOutputBuffer, UINT uNumInputChannels, UINT uNumOutputChannels)
{
	...
	//If delay = 0, just pass input to output
	...
	//Create Wet/Dry mix and write to output buffer
	//Dry = 1 - Wet
	pOutputBuffer[0] = m_fWetLevel*yn + (1.0 - m_fWetLevel)*xn;

	//Increment pointser and wrap if necessary
	m_nWriteIndex++;
...
void CDDLModule::cookVariables() {
	...
	//Subtarct to make read index
	m_nReadIndex = m_nWriteIndex - (int)m_fDelayInSamples;	//Cast as int

	//Check and wrap backwards if if index is positive
	if(m_nReadIndex < 0)
